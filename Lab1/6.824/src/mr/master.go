		...
		for i, _ := range m.mapOps {
			if m.mapOps[i].status == 0 {
				...
				go func(tonuke int) {
					...
					defer m.mu.Unlock()
					m.mu.Lock()
					if m.mapOps[tonuke].status == 1 {
						m.mapOps[tonuke].status = 0
					}
				}(i)
				break
			}
		...
		for i, _ := range m.redOps {
			if m.redOps[i].status == 0 {
				...
				go func(tonuke int) {
					...
					defer m.mu.Unlock()
					m.mu.Lock()
					if m.redOps[tonuke].status == 1 {
						m.redOps[tonuke].status = 0
					}
				}(i)
				break
			}
...
func (m *Master) GiveReport(ra *ReportArgs, rr *ReportReply) error {
	...
	if workAss.operation == 0 {
		...
		m.redDone = true
		for i, _ := range m.redOps {
			if m.redOps[i].status != 2 {
				m.redDone = false
				break
			}
		}
		// fmt.Println(m.redDone)
		rr.WorkerIndex = workerId
		return nil
	}
}
